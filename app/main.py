...
from app.notifier import send_bark_notification, send_dingtalk_notification
...
async def run(dry_run: bool = False, env_file: str | None = None) -> int:
    ...
    await _notify_bark(settings, task.task_id, dry_run, results, screenshots)
...
async def _notify_bark(
    ...
    if not settings.bark_url:
        return
    try:
        await send_bark_notification(settings.bark_url, task_id, dry_run, results, screenshots)
        LOGGER.info("Bark 通知发送成功")
    except Exception:
        LOGGER.exception("Bark 通知发送失败，不影响本次任务结果")
